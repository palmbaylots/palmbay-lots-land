  ...
  "review": [
    {
      ...
      "reviewBody": "Very informative and consistent updates. Vahid connected me with the right people to complete the sale of my property, and he was the foundation that directed the best outcome. My realtor in Washington State researched and found Vahid Rajabian to be the best — he was right!"
    },
    {
      "@type": "Review",
      "reviewRating": { "@type": "Rating", "ratingValue": "5" },
...
const Home = () => {
  ...
  // Real average rating (so the summary stays honest when lower-star reviews are shown).
  const avgRating = reviewCount ? (testimonials.reduce((s, t) => s + (Number(t.rating) || 5), 0) / reviewCount) : 5;
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <div className="flex items-center justify-center gap-2 mb-2">
              {[...Array(5)].map((_, i) => (
                <svg key={i} className={`w-6 h-6 ${i < avgRounded ? 'text-amber-500' : 'text-slate-300'}`} fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              ))}
              <span className="text-slate-600 font-medium ml-1">{avgRatingText} — {reviewCount} Verified Reviews</span>
            </div>
            <a 
                ...
                {[...testimonials, ...testimonials].map((testimonial, index) => (
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center gap-1">
                      {[...Array(5)].map((_, i) => (
                        <svg key={i} className={`w-4 h-4 ${i < (Number(testimonial.rating) || 5) ? 'text-amber-500' : 'text-slate-300'}`} fill="currentColor" viewBox="0 0 20 20">
                          <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                        </svg>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <div>
                      <p className="font-semibold text-slate-900">
                        {(testimonial.name && testimonial.name.trim()) ? testimonial.name : 'Verified Client'}
                        {(testimonial.name && testimonial.name.trim()) && <span className="ml-1.5 text-xs font-medium text-green-600">· Verified Client</span>}
                      </p>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setSelectedReview(null)}>
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6 relative animate-in fade-in zoom-in duration-200" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-1 mb-4">
              {[...Array(5)].map((_, i) => (
                <svg key={i} className={`w-5 h-5 ${i < (Number(selectedReview.rating) || 5) ? 'text-amber-500' : 'text-slate-300'}`} fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                </svg>
              ))}
            </div>
            ...
            <p className="font-semibold text-slate-900">
              {(selectedReview.name && selectedReview.name.trim()) ? selectedReview.name : 'Verified Client'}
              {(selectedReview.name && selectedReview.name.trim()) && <span className="ml-1.5 text-xs font-medium text-green-600">· Verified Client</span>}
            </p>
